...
void QtDialogView::startIndexingDialog(Project* project,
  ...
  m_onQtThread([=, this]() {
    ...
    connect(window, &QtIndexingStartDialog::setShallowIndexing, [this](bool enabled) { m_shallowIndexingEnabled = enabled; });
    ...
    std::function<void(RefreshMode)> onRefreshModeChanged = ([=, this](RefreshMode refreshMode) {
      ...
      connect(timer.get(), &QTimer::timeout, [this]() { showUnknownProgress(L"Preparing Indexing", L"Processing Files", true); });
      timer->start(200);
    ...
    connect(window, &QtIndexingStartDialog::startIndexing, [=, this](RefreshMode refreshMode) {
      ...
      Task::dispatch(TabId::app(), std::make_shared<TaskLambda>([=, this]() { onStartIndexing(info); }));
