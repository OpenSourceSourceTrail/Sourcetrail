...
TEST(FilePathTest, GetInternalPathVariable) {
  ...
  FilePath inst{__FILE__};
  boost::filesystem::path expectedInternalPath{__FILE__};
...
TEST(FilePathTest, CallCustomConstructor) {
  // Give: fileName, currentDir, expectedFilePath
  std::wstring fileName = {__FILE_NAME__, __FILE_NAME__ + sizeof(__FILE_NAME__) - 1};
  std::wstring currentDir = {LIB_TEST_ROOT_DIR, LIB_TEST_ROOT_DIR + sizeof(LIB_TEST_ROOT_DIR) - 1};
  FilePath expectedFilePath{__FILE__};

  // When: call FilePath Constructor
  FilePath inst{fileName, currentDir};

  // Then: inst equals to expectedFilePath
  EXPECT_EQ(inst, expectedFilePath);
}
...
// Given: FilePath inst
#ifdef D_WINDOWS
  _putenv(fmt::format("TEST_FILE={}", __FILE_NAME__));
#else
