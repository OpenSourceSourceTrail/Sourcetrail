...
#include "indexing/data/TaskExecuteCustomCommands.h"
#include "indexing/logic/IndexerCommandProvider.h"
#include "indexing/logic/TaskBuildIndex.h"
#include "indexing/logic/TaskFillIndexerCommandQueue.h"
...
std::shared_ptr<TaskBuildIndex> DefaultTaskFactory::createBuildIndex(size_t processCount,
                                                                     std::shared_ptr<IndexerWorkerServiceImpl> indexerWorkerService,
...
std::shared_ptr<TaskExecuteCustomCommands> DefaultTaskFactory::createExecuteCustomCommands(
  ...
  return std::make_shared<TaskExecuteCustomCommands>(
      std::move(indexerCommandProvider), std::move(storage), std::move(dialogView), indexerThreadCount, std::move(projectDirectory));
}
